...
const VendorController = {
    dashboard: async (req, res) => {
        try {
            ...
            if (!vendor) {
                if (wantsHtml(req)) {
                    return res.redirect(
                        '/?error=' + encodeURIComponent('Vendor not found for this account')
                    );
                }
            ...
            const limit = 10;
            const offset = 0;
            const transactions = await TransactionModel.getRecentByVendor(vendor.id, {
                limit,
                offset,
            });
        ...
        try {
            ...
            if (!vendor) {
                if (wantsHtml(req)) {
                    return res.redirect(
                        Pages.vendor.index.route +
                            '?error=' +
                            encodeURIComponent('Vendor not found')
                    );
                }
                return error(res, 403);
            }
            ...
            if (wantsHtml(req)) {
                return res.redirect(
                    Pages.vendor.index.route +
                        '?error=' +
                        encodeURIComponent('Failed to load checkout')
                );
            }
            return error(res, 500);
        }
        ...
        try {
            ...
            const txn = await TransactionModel.createTransaction({
                ...
                metadata: JSON.stringify({
                    ...
                    vendor_lat:
                        typeof req.body.vendor_lat !== 'undefined'
                            ? req.body.vendor_lat
                            : vendor.latitude || null,
                    vendor_lng:
                        typeof req.body.vendor_lng !== 'undefined'
                            ? req.body.vendor_lng
        ...
        try {
            ...
            if (wantsHtml(req)) {
                return res.redirect(
                    Pages.vendor.index.route +
                        '?error=' +
                        encodeURIComponent('Failed to load item form')
                );
            }
            return error(res, 500);
        }
    ...
    createItem: async (req, res) => {
        try {
            ...
            if (!trimmedName) {
                ...
                if (wantsHtml(req))
                    return res.redirect(
                        Pages.vendor.index.route + '?error=' + encodeURIComponent(msg)
                    );
            ...
            if (newCat) {
                ...
                const row = ItemModel._db().get('SELECT id FROM Categories WHERE name = ?', [
                    newCat,
                ]);
                if (row && row.id) {
            ...
            if (wantsHtml(req)) {
                return res.redirect(
                    Pages.vendor.index.route + '?success=' + encodeURIComponent('Item created')
                );
            }
    ...
    showEditItem: async (req, res) => {
        try {
            ...
            if (wantsHtml(req)) {
                return res.redirect(
                    Pages.vendor.index.route + '?error=' + encodeURIComponent('Failed to load item')
                );
            }
            return error(res, 500);
        }
    ...
    updateItem: async (req, res) => {
        try {
            ...
            if (wantsHtml(req)) {
                return res.redirect(
                    Pages.vendor.index.route + '?success=' + encodeURIComponent('Item updated')
                );
            }
            return res.json({ message: 'ok' });
        } catch (err) {
            ...
            if (wantsHtml(req)) {
                return res.redirect(
                    Pages.vendor.index.route +
                        '?error=' +
                        encodeURIComponent('Failed to update item')
                );
            }
            return res.status(500).json({ error: 'internal error' });
        }
    ...
    deleteItem: async (req, res) => {
        try {
            ...
            if (wantsHtml(req)) {
                return res.redirect(
                    Pages.vendor.index.route + '?success=' + encodeURIComponent('Item deleted')
                );
            }
            return res.json({ message: 'ok' });
        } catch (err) {
            ...
            if (wantsHtml(req)) {
                return res.redirect(
                    Pages.vendor.index.route +
                        '?error=' +
                        encodeURIComponent('Failed to delete item')
                );
            }
            return res.status(500).json({ error: 'internal error' });
        }
    ...
    mapView: async (req, res) => {
        try {
            ...
            if (wantsHtml(req)) {
                return res.redirect(
                    Pages.vendor.index.route + '?error=' + encodeURIComponent('Failed to load map')
                );
            }
            return error(res, 500);
        }
