...
const PaymentsController = {
    ...
    submitPayment: async (req, res) => {
        try {
                ...
                return wantsHtml(req)
                    ? res.redirect(
                          '/payments/pay/' +
                              ...
                              '?error=' +
                              encodeURIComponent('Transaction not found')
                      )
                    : res.status(404).json({ error: 'not found' });
            if (tx.status === 'completed')
                return wantsHtml(req)
                    ? res.redirect(
                          '/payments/success?success=' + encodeURIComponent('Already paid')
                      )
            ...
            if (!payerWallet)
                return wantsHtml(req)
                    ? res.redirect('/wallet?error=' + encodeURIComponent('No wallet'))
                    : res.status(400).json({ error: 'no wallet' });
            ...
            await WalletModel.update(payerWallet.id, {
                balance_tokens: Number(payerWallet.balance_tokens) - amount,
            });
            if (tx.walletDestination_id) {
                const dest = await WalletModel.getById(tx.walletDestination_id);
                if (dest) {
                    await WalletModel.update(dest.id, {
                        balance_tokens: Number(dest.balance_tokens) + amount,
                    });
                }
            }
            ...
            return wantsHtml(req)
                ? res.redirect(
                      '/payments/success?success=' + encodeURIComponent('Payment completed')
                  )
                : res.json({ message: 'ok' });
        } catch (err) {
            console.error(err);
            return wantsHtml(req)
                ? res.redirect('/payments/error?error=' + encodeURIComponent('Payment failed'))
